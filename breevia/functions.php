...
function breevia_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'breevia_register_components', 20 );
Theme::load(
	array(
 ...
 * @return Theme
 */
function breevia_theme() {
	return Theme::getInstance();
}

function breevia_assets() {
	return breevia_theme()->getAssetsManager();
}
...
add_action('after_setup_theme', function() {
    breevia_theme()->register_menus(
        array(
            'header-menu' => esc_html__( 'Header Menu', 'breevia' ),
            'footer-menu' => esc_html__( 'Footer Menu', 'breevia' ),
        )
    );
}, 1);
...
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	breevia_assets()
		...
		->registerStylesheet( 'breevia-theme', '/theme/theme.css' );
	breevia_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		breevia_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action( 'wp_enqueue_scripts', 'breevia_print_color_scheme', 0 );
} else {
	breevia_assets()
		->registerStyle( 'breevia-theme', breevia_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function breevia_theme_add_woocommerce_support() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'breevia_theme_add_woocommerce_support' );

add_filter( 'kubio/activation/after_activation_redirect_url', 'breevia_after_activation_redirect_url' );
function breevia_after_activation_redirect_url( $url ) {
...
function breevia_get_builder_plugin_slug() {
...
function breevia_theme_plugins( $plugins ) {

	$plugin_slug = breevia_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'breevia' ) : __( 'Kubio', 'breevia' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'breevia_theme_plugins' );
...
Hooks::prefixed_add_action(
	...
	function ( $slug ) {
		...
		if ( $slug === breevia_get_builder_plugin_slug() ) {
                ...
                <div class="kubio-admin-row get-started-imported kubio-notice notice notice-success">
                    <div>
                        <p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'breevia' ),
									breevia_theme()->getName()
								)
							);
							?>
                        </p>
                        <p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'breevia' ),
									breevia_theme()->getName()
								)
							);
							?>
                    ...
                    <div class="button imported-view-site-button">
                        <a href="<?php echo esc_url( site_url() ); ?>">
							<?php echo esc_html( __( 'View site', 'breevia' ) ); ?>
                        </a>
...
function breevia_try_online_url() {
	return 'https://kubiobuilder.com/go/try-theme/' . get_template();
}

add_filter( 'kubio/editor-try-online/url', 'breevia_try_online_url', 10 );

function breevia_render_header_style() {

	?>
    <base target="_top">
	<?php

	breevia_theme()->get( 'css' )->render();
}
add_action( 'wp_head', 'breevia_render_header_style', 100 );
