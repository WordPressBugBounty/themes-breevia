...
(function ($) {
  var $notice_container = $(".breevia-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.breevia_admin) {
    return;
  }

  var _window$breevia_admin = window.breevia_admin,
      builderStatusData = _window$breevia_admin.builderStatusData,
      getStartedData = _window$breevia_admin.getStartedData;
  ...
  function toggleProcessing(value) {
    $(window).on("beforeunload.breevia-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".breevia-admin-big-notice").addClass("processing");
      $(".breevia-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".breevia-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.breevia-admin-big-notice");
    });
  }
  ...
  $notice_root = $notice_container.closest(".breevia-admin-big-notice");
  $custom_close_button = $notice_root.find(".breevia-notice-dont-show-container");

  if ($custom_close_button.length) {
    $custom_close_button.on("click", function () {
      disableNotice();
      $notice_container.closest(".breevia-admin-big-notice").find("button.notice-dismiss").click();
    });
  } else {
  ...
  window.breevia_admin.showOverlay = showOverlay;
})(jQuery);
})();
